...
镜像（稳定）:                    业务代码（git 拉取）:
  ...
  ms-swift 4.2.0.dev0             sft_v5.sh / bench_variant.sh 启动脚本
  modelscope CLI                  docs/
...
3. [sft_v5.sh 完整选项](#3-sft_v5sh-完整选项)
...
```bash
bash sft_v5.sh status     # 看容器状态、最新 step
bash sft_v5.sh logs       # docker logs -f
bash sft_v5.sh stop       # 停掉
```
...
完成 2.1-2.8 后机器就 ready 了，回到 [§1 一句话启动训练](#1-一句话启动训练) 开训。
...
| 命令 | 作用 |
|---|---|
| `bash sft_v5.sh` 或 `bash sft_v5.sh train` | 默认：full 训练（按 `NUM_EPOCHS` 配置）|
| `bash sft_v5.sh smoke` | 5 步 smoke test，~5 min，前台跑，验证 step 1 数值 |
| `bash sft_v5.sh status` | 查看最近一次 run 的容器状态 + 最新 step |
| `bash sft_v5.sh logs` | `docker logs -f` 跟随最近一次 run |
| `bash sft_v5.sh stop` | `docker stop` 最近一次 run |
...
> 如果想用 main 上的最新（未充分验证）：`CODE_REF=main bash sft_v5.sh`。
...
│   └── runs/               # tensorboard
└── .meta/latest_run.txt    # 最近一次 run 元信息（status/logs/stop 用这个）
```

### 3.4 开发者模式（自动检测）

如果你在 `git clone` 出来的仓库目录里跑 `bash scripts/gemma4_E4B_opt/sft_v5.sh`，脚本会**自动 mount 你的本地 repo** 到容器 `/workspace/code`，**跳过 git clone**——你改业务代码不需要 git push 就能立即生效。这是开发新 patch 时最常用的方式。
...
| **启动脚本（推荐）** | [`scripts/gemma4_E4B_opt/sft_v5.sh`](../scripts/gemma4_E4B_opt/sft_v5.sh) |
