...
> 镜像（仅含依赖，业务代码 git 拉取）：`fangyaohua/gemma4-e4b-it-sft:runtime-260506-u22.04-cu12.9.1-py3.12-t2.10.0-v0.19.0-m1.35.4-s4.1.2-r1`
>
> 业务代码：`https://github.com/fyh2001/megatron-sft-recipes.git` @ `v1.0`
>
> 一句话：**对齐 DeepSpeed ZeRO-3 baseline（mean Δloss +0.044%），比 DS3 快 1.36 倍**（H100 8 卡 2 epoch：9.0 h vs 12.3 h）。

---

## 镜像设计哲学（重要先看）

旧设计把所有东西打进镜像，patch 改一下要 rebuild + push 30 GB。新设计：

```
镜像（稳定）:                    业务代码（git 拉取）:
  PyTorch 2.10                    sitecustomize.py（运行时 monkey-patches）
  CUDA 12.9                       swift gemma.py buffer-fix patch
  ms-swift 4.2.0.dev0             sft_fsdp.sh / bench_variant.sh 启动脚本
  modelscope CLI                  docs/
  git, patch, entrypoint.sh       分析工具 (plot/cossim/...)
```

容器启动时 entrypoint 自动按 `CODE_REPO` + `CODE_REF` 拉代码、apply patches、设 PYTHONPATH，**对用户完全透明**。

业务代码改了 → `git push` + 改 `CODE_REF` 即可，**不用 rebuild 镜像**。
...
3. [sft_fsdp.sh 完整选项](#3-sft_fsdpsh-完整选项)
...
```bash
# 机器装好 driver + docker + nvidia-toolkit + 模型 + 数据后
...
bash sft_fsdp.sh                  # 默认 train，2 epoch ~9h on 8x H100
```

启动后会在 `runs/sft_v5_<timestamp>/` 下产出 checkpoints + logs，**默认后台跑**（`RUN_IN_BACKGROUND=true`），脚本启动后会打印：

```
[info] view logs : docker logs -f gemma4-e4b-sft-v5-<RUN_TS>
[info] stop      : docker stop gemma4-e4b-sft-v5-<RUN_TS>
[info] output    : .../runs/sft_v5_<RUN_TS>
```

如果想前台跑（直接看到训练日志），把 `RUN_IN_BACKGROUND=false` 即可。
...
docker run --rm \
  ...
  fangyaohua/gemma4-e4b-it-sft:runtime-260506-u22.04-cu12.9.1-py3.12-t2.10.0-v0.19.0-m1.35.4-s4.1.2-r1 \
  -c "modelscope download --model google/gemma-4-E4B-it --local_dir /root/.cache/modelscope/models/google/gemma-4-E4B-it"
```

下载完成后模型在 host 的 `$HOME/.cache/modelscope/models/google/gemma-4-E4B-it/`。

> 注意 `--entrypoint /bin/bash`：新镜像的默认 entrypoint 会 git clone 业务代码，下载模型时不需要这步，所以用 entrypoint override 跳过。

### 2.6 下载 sft_fsdp.sh launcher
...
完成 2.1-2.7 后机器就 ready 了，回到 [§1 一句话启动训练](#1-一句话启动训练) 开训。第一次跑训练时容器会 git clone v1.0 + apply swift gemma.py patch，约 30 秒，之后 swift sft 启动时 stderr 应该出现：

```
[INFO:swift] [Gemma4Loader] manually loaded 42 buffer(s) (layer_scalar / std_scale / std_bias) ...
```

step 1 应该输出 `loss=2.226 grad_norm=10.28`，说明数值跟 v5 全跑完全对齐。

---

## 3. sft_fsdp.sh 完整选项

### 3.1 配置（直接在脚本顶部 `[USER CONFIG]` 段编辑）

#### 3.1.1 必填 4 项（90% 用户只动这几个）

| 配置 | 默认 | 含义 |
|---|---|---|
| `IMAGE` | `fangyaohua/gemma4-e4b-it-sft:runtime-...-r1` | 训练镜像 tag |
| `MODEL_HOST_DIR` | `$HOME/.cache/modelscope/models/google/gemma-4-E4B-it` | host 上的模型目录 |
| `DATA_HOST_PATH` | `$(pwd)/sft.jsonl` | host 上的训练 jsonl |
| `OUTPUT_HOST_DIR` | `$(pwd)/runs` | host 上输出目录 |

#### 3.1.2 业务代码版本（用 git tag 锁定）

| 配置 | 默认 | 含义 |
|---|---|---|
| `CODE_REPO` | `https://github.com/fyh2001/megatron-sft-recipes.git` | 业务代码仓库 |
| `CODE_REF` | `v1.0` | git ref（tag / branch / commit hash）|

> `v1.0` 是经过 800 步全跑验证的稳定版本，**生产推荐**。
> 想用 main 上的最新（未充分验证）：把 `CODE_REF=v1.0` 改成 `CODE_REF=main`。
...
| `GRAD_ACCUM_STEPS` | `16` | GAS（GBS = NPROC × MBS × GAS = 128）|
| `MAX_LEN` | `16384` | sequence 长度 |
| `SAVE_STRATEGY` | `epoch` | `epoch` / `steps` / `no` |
| `SEED` / `DATA_SEED` | `42` / `42` | 复现性种子 |
| `RUN_IN_BACKGROUND` | `true` | true=后台跑（docker logs 看进度）；false=前台 |

#### 3.1.4 v5 内部（强烈不建议改）

| 配置 | 默认 | 含义 |
|---|---|---|
| `TORCH_DTYPE` | `float32` | fp32 sharded master |
| `USE_BF16_MP` | `true` | bf16 mixed precision compute |
| `PADDING_FREE` | `false` | Gemma-4 必须 false |
| `ACTIVATION_CPU_OFFLOAD` | `true` | patch 21b 需要 |
| `GEMMA4_FSDP_REDUCE_FP32_NCCL` | `1` | patch 21 + 21b（fp32 NCCL）|
| `GEMMA4_KV_SHARE_DETACH` | `1` | patch 8（KV-share .detach）|

### 3.2 输出文件结构

`runs/sft_v5_<timestamp>/` 下产出：

```
sft_v5_<TS>/
├── train.log               # host-tee 训练日志
├── fsdp_override.json      # 容器内自动写的 FSDP2 配置
└── v0-<TS>/
    ├── args.json           # swift 完整训练参数
    ├── logging.jsonl       # 每步 metrics（loss/grad_norm/...）
    ├── checkpoint-403/     # epoch 1 末
...
如果你在 `git clone` 出来的仓库目录里跑 `bash scripts/gemma4_E4B_opt/sft_fsdp.sh`，脚本会**自动 mount 你的本地 repo** 到容器 `/workspace/code`，**跳过 git clone**——你改业务代码不需要 git push 就能立即生效。这是开发新 patch 时最常用的方式。
...
| 镜像 tag | `260506-u22.04-cu12.9.1-py3.12-t2.10.0-v0.19.0-m1.35.4-s4.1.2-v2` |
| 镜像 size | ~10 GB compressed (runtime-only) |
| Dockerfile | [`docker/Dockerfile`](../docker/Dockerfile) |
| 容器入口 | [`docker/entrypoint.sh`](../docker/entrypoint.sh) |
| 准备脚本 | [`docker/prepare.sh`](../docker/prepare.sh) |
| **启动脚本（推荐）** | [`scripts/gemma4_E4B_opt/sft_fsdp.sh`](../scripts/gemma4_E4B_opt/sft_fsdp.sh) |
| 旧启动脚本（自包含）| [`scripts/gemma4_E4B_opt/fsdp2_offload_E4B_text_only_2ep_a3_pf_fp32master.sh`](../scripts/gemma4_E4B_opt/fsdp2_offload_E4B_text_only_2ep_a3_pf_fp32master.sh) |
