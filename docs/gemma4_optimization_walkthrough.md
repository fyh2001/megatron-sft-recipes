...
每个 phase 目录（`megatron_output/gemma4_opt/p${N}_${axis}/`）下：
...
docker exec fsdp_sft bash -lc "cd /home/ubuntu/fyh/megatron-sft-recipes && \
  ...
  BENCH_DIR=/home/ubuntu/fyh/megatron_output/gemma4_opt/p0_baseline_fsdp2 \
...
>
> **13 次 attempt** 把 FSDP2 + offload + 26B + seq=16384 + GBS=64 + Ulysses SP=2 在 80 GB H100 上跑通：详见 [attempts.md](../../megatron_output/gemma4_opt/p0_train_align/attempts.md)。最终配方 6 件套见 §0g 末尾。
>
...
数字落在 [`p0_train_align/_baselines_misaligned/compare.{tsv,txt}`](../../megatron_output/gemma4_opt/p0_train_align/_baselines_misaligned/)。
...
python /home/ubuntu/fyh/megatron-sft-recipes/scripts/benchmark/compare_loss_curves.py \
    --a /home/ubuntu/fyh/megatron_output/gemma4_opt/p0_train_align/run_*/logging.jsonl \
    --a-label fsdp2_align \
    --b /home/ubuntu/fyh/megatron_output/gemma4_opt/p0_baseline_ds_prod/run_20260424_221109_full_epoch/swift_out/v0-20260425-061132/logging.jsonl \
    --b-label ds_baseline \
    --max-steps 40 \
    --out /home/ubuntu/fyh/megatron_output/gemma4_opt/p0_train_align/run_*/compare_vs_ds
```
...
详见 [attempts.md](../../megatron_output/gemma4_opt/p0_train_align/attempts.md)。简表：
...
完整 sweep 矩阵（7 个点）见 [scripts/gemma4_opt/p1_gbs_sweep.sh](../scripts/gemma4_opt/p1_gbs_sweep.sh) `SWEEP=(...)` 数组。每点 30 步（5 warmup + 25 measure），结果落 `megatron_output/gemma4_opt/p1_gbs_sweep/run_<TS>_<LABEL>/`。
...
**4. MBS=2 因 swift Ulysses bug 不可达**：[attempts.md](../../megatron_output/gemma4_opt/p1_gbs_sweep/attempts.md) 详记。`swift/sequence_parallel/ulysses.py:490` 假设 `attention_mask` 是 tensor，但 gemma4 VLM 是 dict。MBS=1 走 single-sample path OK，MBS≥2 batched collation 触发 bug。短期 workaround：锁定 MBS=1。

### 1.5 Debug 记录 — sweep 工程踩坑

详见 [attempts.md](../../megatron_output/gemma4_opt/p1_gbs_sweep/attempts.md) "Pre-cleanup failed attempts" 表格。简表：
...
- ✅ 每期所有 run（成功 + 失败）保留在 megatron_output/gemma4_opt/p*/run_*
