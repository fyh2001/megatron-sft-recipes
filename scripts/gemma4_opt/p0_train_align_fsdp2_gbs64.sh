...
#   - --optim adamw_torch: device-agnostic torch.optim.AdamW.  With FSDP
#     CPUOffloadPolicy, params/grads live on CPU; torch AdamW will then run
#     opt-state allocation + step math on CPU, keeping GPU footprint near
#     zero for opt state.  Numerically identical to bnb paged_adamw_32bit
#     (both fp32 master + m + v).  We tried paged_adamw_32bit first (run_09)
#     but bnb's prefetch_state hits 'NoneType deviceid' when params are on
#     CPU (DTensor offloaded shards have no device.index) — bnb expects CUDA
#     params.  adamw_torch sidesteps this entirely.
...
# steps.  For FSDP2 align run to share the SAME lr at every step (during the
# first 15 steps, i.e. the warmup phase), we override warmup_steps=15 directly
# (not warmup_ratio, which would compute 40*0.05=2 with our shorter max_steps).
# After step 15 lr decay denominators differ (40 vs 294 total) — comparison is
# only valid in the warmup window.
