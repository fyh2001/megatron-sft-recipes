...
for entry in "${SWEEP[@]}"; do
    ...
    echo "=========================================="
    echo "P1 sweep [${LABEL}]: MBS=${MBS} GAS=${GAS} → GBS=${GBS}"
...
MASTER_PORT=29555 BACKEND=fsdp2 SP=2 MBS=${MBS} GAS=${GAS} \
...
" 2>/dev/null)"
    ...
    # Always cleanup leftover swift / torchrun children to free GPU before next config
    docker exec fsdp_sft pkill -9 -f "swift|torch.distributed.run|sft.py" 2>/dev/null || true
    sleep 5
done
