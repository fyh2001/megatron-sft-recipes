...
#   - swap --deepspeed → --fsdp <override.json> (with "offload" → params+grads to CPU)
#   - drop --gradient_checkpointing true (FSDP2 AC lives in fsdp_config.activation_checkpointing)
#   - add --optim paged_adamw_32bit: accelerate's FSDP cpu_offload only offloads
#     params+grads, NOT optimizer state.  AdamW master+m+v fp32 sharded = ~39 GB
#     per rank for 26B/8 ranks; combined with ~8.6 GB SDPA attn matrix (gemma4
#     global head_dim=512 fallback) → 80 GB OOM.  bnb PagedAdamW32bit puts opt
#     state in CPU pinned mem with CUDA UM auto-paging; numerically identical to
#     torch AdamW (NOT 8-bit quantised), preserving loss-curve alignment.
#     This mirrors DS prod baseline's `offload_optimizer.device=cpu`.
...
# NOTE: "offload" in the fsdp string → transformers/training_args.py:2760 sets
# fsdp_plugin_args["cpu_offload"]=True → accelerate FSDP2 wraps with
# CPUOffloadPolicy(), offloading sharded params (and optimizer state via
# PyTorch's mechanism) to host RAM.  Without this, FSDP2's idle resident GPU
# state for AdamW (master+m+v fp32 + bf16 model + bf16 grads = ~52 GB/rank for
# 26B/8 ranks) leaves no room for gemma4 GLOBAL-attn SDPA matrix
# (1×16×16384²×bf16 = 8.6 GiB/layer, halved by SP=2 → 2.15 GiB but with
# math-mode intermediate copies → 4.27 GiB tried in run_02 OOM).
# DS prod baseline gets the same headroom via offload(opt+param) CPU; here we
# get the equivalent on FSDP2 via the same accelerate flag.
cat > "${FSDP_OVERRIDE}" <<'EOF'
{
    "_purpose": "FSDP2 align run mirroring DS prod-baseline. ZeRO-3 (full reshard) + AC=on + CPU offload (params+opt), gemma4-specific workarounds.",
